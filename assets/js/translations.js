const translations = {
    "en": {
        "appTitle": "LeelaClue",
        "disclaimerTitle": "Disclaimer",
        "termsOfUseTitle": "Terms of Use",
        "impressumTitle": "Impressum",
        "contactTitle": "Support",
        "readManualBtn": "Read User Guide",
        "userGuideTitle": "User Guide",
        "privacyPolicyTitle": "Privacy Policy",
        "iosComingSoon": "iOS version coming soon",
        "googlePlay": "Get it on Google Play",
        "blogTitle": "Blog",
        "blog_post_1_title": "The Psychology of Choice: How LeelaClue Works",
        "blog_post_2_title": "Leela: Why I Facilitate This Game – And Why It Can Change Your Life",
        "blog_post_3_title": "The Soul's Map: How Leela Works",
        "blog_post_4_title": "From Intuition to Algorithm: How LeelaClue Was Born",
        "blog_post_5_title": "The Language of the Field: A Reflection on Truth, Stillness, and New Beginnings",
        "blog_post_6_title": "The St-O-R Framework: Your Intuition Has the Answer. Your Brain Won't Quit.",
        "navGuide": "Guide",
        "navFAQ": "FAQ",
        "navCommunity": "Community",
        "navNews": "Release News",
        "navVotes": "Feature Votes",
        "navOfflineGame": "Offline Game",
        "navLegal": "Legal",
        "navSupport": "Support",
        "privacyNotice": "We use cookies and local storage to provide functional features like language preferences and voting. We do not use tracking or collect personal data.",
        "privacyAccept": "Got it",
        "webPrivacyTitle": "Website Privacy",
        "newBadge": "NEW",
        "latestBlogPrefix": "Latest:",
        "newBlogLabel": "New Blog"
    },
    "de": {
        "appTitle": "LeelaClue",
        "disclaimerTitle": "Haftungsausschluss",
        "termsOfUseTitle": "Nutzungsbedingungen",
        "impressumTitle": "Impressum",
        "contactTitle": "Support",
        "userGuideTitle": "Benutzerhandbuch",
        "privacyPolicyTitle": "Datenschutzerklärung",
        "readManualBtn": "Handbuch Lesen",
        "iosComingSoon": "iOS-Version bald verfügbar",
        "googlePlay": "Jetzt bei Google Play",
        "blogTitle": "Blog",
        "blog_post_1_title": "Die Psychologie der Entscheidung: So funktioniert LeelaClue",
        "blog_post_2_title": "Leela: Warum ich dieses Spiel leite – und warum es dein Leben verändern kann.",
        "blog_post_3_title": "Die Landkarte deiner Seele: Wie Leela (Lila) funktioniert",
        "blog_post_4_title": "Von der Intuition zum Algorithmus: Wie LeelaClue geboren wurde.",
        "blog_post_5_title": "Die Sprache des Feldes: Eine Reflektion über Wahrheit, Stille und Neubeginn.",
        "blog_post_6_title": "Das St-O-R-Modell: Deine Intuition kennt die Antwort. Dein Verstand gibt einfach keine Ruhe.",
        "navGuide": "Handbuch",
        "navFAQ": "FAQ",
        "navCommunity": "Community",
        "navNews": "Releases",
        "navVotes": "Features Voten",
        "navOfflineGame": "Offline Spiel",
        "navLegal": "Rechtliches",
        "navSupport": "Support",
        "privacyNotice": "Wir verwenden Cookies und lokalen Speicher für funktionale Zwecke, wie Spracheinstellungen und Abstimmungen. Wir verwenden kein Tracking und sammeln keine persönlichen Daten.",
        "privacyAccept": "Verstanden",
        "webPrivacyTitle": "Website-Datenschutz",
        "newBadge": "NEU",
        "latestBlogPrefix": "Neu:",
        "newBlogLabel": "Neuer Blog"
    },
    "ru": {
        "appTitle": "LeelaClue",
        "disclaimerTitle": "Отказ от ответственности",
        "termsOfUseTitle": "Условия использования",
        "impressumTitle": "Импрессум",
        "contactTitle": "Поддержка",
        "userGuideTitle": "Руководство пользователя",
        "privacyPolicyTitle": "Политика конфиденциальности",
        "readManualBtn": "Читать Руководство",
        "iosComingSoon": "iOS версия скоро",
        "googlePlay": "Доступно в Google Play",
        "blogTitle": "Блог",
        "blog_post_1_title": "Психология выбора: Как работает LeelaClue",
        "blog_post_2_title": "Лила: Почему я провожу эту игру — и почему она может изменить твою жизнь.",
        "blog_post_3_title": "Карта твоей души: Как работает Лила",
        "blog_post_4_title": "От интуиции к алгоритму: как родилась LeelaClue",
        "blog_post_5_title": "Язык Поля: Размышление об истине, тишине и новом начале.",
        "blog_post_6_title": "Модель St-O-R: Твоя интуиция знает ответ. Твой ум просто не умолкает.",
        "navGuide": "Инфо",
        "navFAQ": "FAQ",
        "navCommunity": "Сообщество",
        "navNews": "Новости",
        "navVotes": "Голосование",
        "navOfflineGame": "Оффлайн-игра",
        "navLegal": "Юр. инфо",
        "navSupport": "Поддержка",
        "privacyNotice": "Мы используем файлы cookie и локальное хранилище для обеспечения работы таких функций, как языковые настройки и голосование. Мы не используем отслеживание и не собираем личные данные.",
        "privacyAccept": "Понятно",
        "webPrivacyTitle": "Конфиденциальность сайта",
        "newBadge": "НОВОЕ",
        "latestBlogPrefix": "Новое:",
        "newBlogLabel": "Новый блог"
    }
};
